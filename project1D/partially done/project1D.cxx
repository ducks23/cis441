...
//find the middle vertex to split the triangle in half
       ...
       double _m1, _m2, _b1, _b2, _leftEnd, _rightEnd, _leftColor[3], _rightColor[3], _leftDepth, _rightDepth;
       ...
       for(double r1 = rowMin ; r1 <= midLine; r1++){
           ...
           for(c = _leftEnd; c <= _rightEnd; c++){
               ...
               if(c >= 0 && r1 >= 0 && r1 < 1000 && c < 1000){
                   ...
                   buf[0] = (unsigned char) ceil441(255.0 *interp(_leftColor[0], _rightColor[0], _leftEnd, _rightEnd, c));
                   buf[1] = (unsigned char) ceil441(255.0 * interp(_leftColor[1], _rightColor[1], _leftEnd, _rightEnd, c));
                   buf[2] =(unsigned char) ceil441(255.0 *  interp(_leftColor[2], _rightColor[2], _leftEnd, _rightEnd, c));
       ...
       for(int r1 = midLine+1; r1 <= rowMax; r1++){
           ...
           for(c = leftEnd; c <= rightEnd; c++){
               if(c >= 0 && r1 >= 0 && r1 < 1000 && c < 1000){
                   unsigned char *buf = (unsigned char *) image->GetScalarPointer(c,r1,0);
                   buf[0] = (unsigned char) ceil441(255.0 * interp( _leftColor[0] , _rightColor[0], leftEnd, rightEnd, c)); 
                   buf[1] = (unsigned char) ceil441(255.0 * interp( _leftColor[1] , _rightColor[1], leftEnd, rightEnd, c)); 
                   buf[2] = (unsigned char) ceil441(255.0 * interp( _leftColor[2] , _rightColor[2], leftEnd, rightEnd, c)); 
